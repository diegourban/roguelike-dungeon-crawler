...
var RogueLikeDungeonCrawlerGameBoard = React.createClass({
  displayName: 'RogueLikeDungeonCrawlerGameBoard',
    ...
    for (var i = 0; i < open.length; i++) {
      ...
      if (open[i].carve) {
        ...
        while (carveCount < carveNum) {
          ...
          if ((open[i].y - 1 <= 1 || newBoard[open[i].y - 1][open[i].x].type !== 0) && (open[i].x - 1 <= 1 || newBoard[open[i].y][open[i].x - 1].type !== 0) && (open[i].y + 1 >= this.state.height - 2 || newBoard[open[i].y + 1][open[i].x].type !== 0) && (open[i].x + 1 >= this.state.width - 2 || newBoard[open[i].y][open[i].x + 1].type !== 0)) {
            open[i].carve = false;
            carveCount = carveNum;
          }
  ...
  render: function () {
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          Modal,
          { show: this.state.showDeathModal, bsSize: 'sm' },
          React.createElement(
            Modal.Body,
            null,
            React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              React.createElement(
                Button,
                { bsStyle: 'danger', onClick: this.restartGame.bind(null, false) },
                'Restart'
              )
            )
          )
        ...
        React.createElement(
          Modal,
          { show: this.state.showWinModal, bsSize: 'sm' },
          React.createElement(
            Modal.Body,
            null,
            React.createElement(
      ...
      React.createElement(Board, { board: this.state.board, player: this.state.player, objectPos: this.state.objectPos, onKeyPress: this.movePlayer, isFog: this.state.fog }),
      React.createElement(
        'div',
        null,
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              'div',
              { className: 'panel panel-default', id: 'char-exp-panel' },
              React.createElement(
                'div',
                { className: 'panel-heading' },
                'XP'
            ...
            React.createElement(
              'div',
              { className: 'panel panel-default', id: 'char-hp-panel' },
              React.createElement(
                'div',
                { className: 'panel-heading' },
                'Health'
            ...
            React.createElement(
              'div',
              { className: 'panel panel-default', id: 'char-attack-panel' },
              React.createElement(
                'div',
                { className: 'panel-heading' },
                'Attack'
          ...
          React.createElement(
            Button,
            { bsStyle: 'danger', onClick: this.toggleFog },
            'Fog ',
            this.state.fog ? 'Off' : 'On'
          )
        )
      )
    );
  ...
  printBoard: function () {
    ...
    var printRows = this.props.board.map(function (row, j) {
      var printCells = row.map(function (cell, i) {
        ...
        return React.createElement('div', { className: "cell " + cellColor + " " + isRevealed, key: cellID, dangerouslySetInnerHTML: { __html: icon } });
      }.bind(this));
      return React.createElement(
        'div',
        { className: 'row', key: j },
...
ReactDOM.render(React.createElement(RogueLikeDungeonCrawlerGameBoard, null), document.getElementById("roguelike-dungeon-crawler"));
